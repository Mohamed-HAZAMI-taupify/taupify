...
import { loginEverestSuperAdmin, logout } from "../../redux/actions/authActions";
...
const SuperAdminLogin = (props) => {
  ...
  useEffect(() => {
    if (isAuthenticatedEverest) {
      window.location.href = data.admin_path + data.superAdmin ;
    }
  ...
  const onSubmit = async (event) => {
    ...
    props.loginEverestSuperAdmin(formData);
  };
...
const mapDispatchToProps = (dispatch) => ({
  loginEverestSuperAdmin: (email, password) => {
    dispatch(loginEverestSuperAdmin(email, password));
  },
  logout: () => {
    dispatch(logout());
  },
